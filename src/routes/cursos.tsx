...
function CursosTab({
  ...
  const fetchCursos = async () => {
    setLoading(true);
    try {
      // API mais limpa: o backend já identifica o tenant_id pelo token (authHeaders)
      ...
      data.forEach((c: any) => {
        ...
        // Se o curso estiver vinculado a um usuário, adiciona na lista
        if (c.usuarios) {
          agrupados[key].usuarios_atribuidos.push({
            ...
            // O front agora verifica se a string de status é igual a 'concluido'
